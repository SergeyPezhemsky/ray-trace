...
bool Parallel::Intersect(const Ray& ray, float tmin, float tmax, SurfHit& surf) const
{
	...
	if (tMin < tMax && tMax > 0 && surf.t > tmin && surf.t < tmax) {
		...
		surf.hitPoint = ray.o + surf.t * ray.d;
		float EPS = 0.001;
		if (fabs(surf.hitPoint.x - t_min.x) < EPS)
			surf.normal = float3(-1, 0, 0);
		else if (fabs(surf.hitPoint.x - t_max.x) < EPS)
			surf.normal = float3(1, 0, 0);
		else if (fabs(surf.hitPoint.y - t_min.y) < EPS)
			surf.normal = float3(0, -1, 0);
		else if (fabs(surf.hitPoint.y - t_max.y) < EPS)
			surf.normal = float3(0, 1, 0);
		else if (fabs(surf.hitPoint.z - t_min.z) < EPS)
			surf.normal = float3(0, 0, -1);
		else if (fabs(surf.hitPoint.z - t_max.z) < EPS)
			surf.normal = float3(0, 0, 1);
